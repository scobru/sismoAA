...
class SismoPKP {
  ...
  async createPassKey(
    ...
    const signer = this.externalProvider;
    const sismoPKPContract = new Contract(
      this.contractAddress,
      SismoABI.abi,
      signer
    );
    ...
    const tx = await sismoPKPContract.createPassKey(
      params.sismoConnectResponse,
      params.appId,
      params.otp
    );
  ...
  async getPassKey(sismoConnectResponse: string, appId: string, otp: string) {
    ...
    const passKey = await sismoPKPContract.getPassKey(
  ...
  async prepareSismoConnect(_appId: string) {
    ...
    const AUTHS: AuthRequest[] = [
      { authType: AuthType.VAULT, isSelectableByUser: false },
    ];
